...
public class AccountWriter implements DatabaseWriter<AccountDTO> {
    ...
    public Mono<Void> write(AccountDTO dto) {
        ...
        DatabaseClient.GenericExecuteSpec spec = r2dbcEntityTemplate.getDatabaseClient().sql(sqlInsert)
        ...
        if (dto.getVatCodeId() != null) {
            spec = spec.bind("vat_code_id", dto.getVatCodeId());
        } else {
            spec = spec.bindNull("vat_code_id", UUID.class);
        }
